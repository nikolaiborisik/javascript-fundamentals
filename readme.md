...
- Установить nodejs, предпочтительнее установить LTS версию https://nodejs.org/en/ Если у вас есть установлена node.js пропустите этот шаг
- Форкнуть данный проект https://help.github.com/en/articles/fork-a-repo
- После того как вы форкнули проект, он будет добавлен к вам на github-e Склонируйте проект со своего профиля
...
Выполните в консоле команду `npm run test#.#` где #.#  подгруппы задания
Пример для 1.1 Базовые типы данных следует выполнить `npm run test1.1`

Для того что бы выполнить только конкретный тест из файла можно после it добавить only
Пример
...
Так же можно использовать встроенные средства для работы с тестами в вашем редакторе
...
Что бы запустить тесты следует выполнить команду jest example.test.js
Результатом будет два не пройденых теста
...
- Задать минимум 5 числовых переменных со следующими значениями: целое, дробное. NaN, бесконечность,
  близкое к максимально безопасному целому. В качестве значение можно использовать выражения

- Написать несколько выражений с 2 операндами и базовыми операторами: сложение, вычитание, умножение и деление

- Написать несколько выражения с 3-ми и более операндами и базовыми операторами: сложение, вычитание, умножение и деление
...
- Обявить три переменные со значениями null и undefined и без значения. Определить их типы
...
- Создать функцию использую Объявление функции (Function Declaration)
...
- Создать функции которая возвращает строку, в которой каждый аргумент с двух сторон обрамлен “|”
...
- Написать несколько примеров округления к ближайшему целому с использование Math и без
- Написать несколько примеров округления к наименьшему целому с использование Math и без
...
- Найти несколько способ исправления кода
...
- Функция должны оборачивать другую функцию Результат должен вычисляться только при первом вызове функции. При последующих вызовах должен возвращаться результат первого вызова без перерасчетов
- Каринг Создать функцию, которая принимает другую функцию F от двух аргрументов и возвращает функцию от одного аргумента. Которая при вызове возвращает фунцию от одного аргумента и при вызове возвращает результат выполнения функции F от двух аргументов
...
- Функция конструктор для создание пользователя с именем и сообщением приветствия через прототип
- Создать ArticleList c двумя методами добавление статьи и получения количества статей с использованием прототипа
- Создать класс Component c 3-мя методами setData(obj), getData() и render(). setData() должен сливать (merge) существующие данные с переданными.
  ...
  Переопределить метод render Так что бы он возвращал строку `${msg}, ${name}!` используя `this.data` Eсли name null или undefined вместо имени использовать guest.
...
- Найти 3 самых больших элемена в массиве
...
- Найти пересечении двух массивов
